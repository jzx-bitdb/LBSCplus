...
void LBSCCache::update_stat_periodic(){
    ...
    cerr
            ...
            << "admit_inference: "<<admit_inference<<endl
...
bool LBSCCache::exist(const int64_t &key) {
    return key_pos.find(key) != key_pos.end();
}



bool LBSCCache::rank_admit(const shared_ptr<Meta>& meta_new, const SimpleRequest &req) {
    // 如果模型还没有训练完成，先全都接纳
    if (!have_trained) {
        // 模型还没训练，先全都接纳
        return true;
    ...
    // ===============================
    // 为新请求对象创建元数据
//     auto meta_new = make_shared<Meta>(req.id, req.size, req.cost, current_seq, req.extra_features); // 创建新的Meta对象
    ...
    // 调整分数：加上对象大小的对数，减去成本的对数
    score_new += log1p(meta_new->_size) - log1p(meta_new->_cost);

    // cerr << "[rank_admit] new score: " << score_new << endl;
    ...
    vector<shared_ptr<Meta>> sample_metas;
    unordered_set<uint64_t> key_set;

    uint32_t tmp_sample_rate_up = in_cache_metas_threshold_up.size() * sample_size / in_cache_metas.size() + 1;

    while (sample_metas.size() < sample_size) {  // 当采样对象数量还没达到目标采样大小时继续循环
        shared_ptr<Meta> meta;  // 声明指向Meta对象的智能指针
        KeyMapEntryT pos;  // 声明键映射条目，用于记录位置信息

        if (is_cost_size_sample) {  // 如果启用了成本/大小采样策略
            if (is_dynamic_cost_size_threshold) {  // 如果启用了动态成本/大小阈值
                if (tmp_evict_num < evict_num_threshold) {  // 如果临时驱逐次数未超过驱逐阈值
                    if (tmp_sample_rate_up == 0) {  // 如果高阈值采样率为0
                        uint32_t posTmp = _distribution(_generator) % in_cache_metas_threshold_low.size();  // 从低层阈值列表中随机选择一个位置
                        pos.list_idx = 0;  // 设置列表索引为0，表示低层阈值列表
                        pos.list_pos = posTmp;  // 记录在低层列表中的位置
                        meta = in_cache_metas_threshold_low[posTmp];  // 从低层阈值列表中获取对应位置的元数据
                    } else {  // 如果高阈值采样率不为0
                        uint32_t posTmp = _distribution(_generator) % in_cache_metas_threshold_up.size();  // 从上层阈值列表中随机选择一个位置
                        pos.list_idx = 1;  // 设置列表索引为1，表示上层阈值列表
                        pos.list_pos = posTmp;  // 记录在上层列表中的位置
                        meta = in_cache_metas_threshold_up[posTmp];  // 从上层阈值列表中获取对应位置的元数据
                        tmp_sample_rate_up--;  // 减少高层阈值采样率计数
                    }
                } else {  // 如果临时驱逐次数超过阈值
                    uint32_t posTmp = _distribution(_generator) % in_cache_metas_threshold_up.size();  // 从上层阈值列表中随机选择一个位置
                    pos.list_idx = 1;  // 设置列表索引为1，表示上层阈值列表
            ...
            } else {  // 如果没有启用动态成本/大小阈值
                uint32_t posTmp = _distribution(_generator) % in_cache_metas_threshold_up.size();  // 从上层阈值列表中随机选择一个位置
                pos.list_idx = 1;  // 设置列表索引为1，表示上层阈值列表
                pos.list_pos = posTmp;  // 记录在上层列表中的位置
                meta = in_cache_metas_threshold_up[posTmp];  // 从上层阈值列表中获取对应位置的元数据
            }
        } else {  // 如果没有启用成本/大小采样策略
            uint32_t posTmp = _distribution(_generator) % in_cache_metas.size();  // 从内部缓存元数据列表中随机选择位置
            pos.list_idx = 0;  // 设置列表索引为0，表示内部缓存
            pos.list_pos = posTmp;  // 记录在内部缓存中的位置
            meta = in_cache_metas[posTmp];  // 从内部缓存中获取对应位置的元数据
        }

         // 如果使用衰减策略，则避免对同一键重复采样
        if(is_use_decay){
            if (key_sets.find(meta->_key) != key_sets.end()) {
                continue;
            } else {
        ...
        sample_metas.push_back(meta);  // 将采样到的元数据添加到采样列表中
    }
    ...
    // ===============================
    // 初始化批量预测用的CSR格式数组
    int32_t indptr[sample_size + 1];
    indptr[0] = 0;
    int32_t indices[sample_size * n_feature];
    double data[sample_size * n_feature];
    vector<uint32_t> sizes(sample_size);
    vector<uint32_t> costs(sample_size);
    ...
    for (auto &meta : sample_metas) {
        ...
        for (uint8_t k = 0; k < n_edc_feature; ++k) {
            indices[idx_feature] = max_n_past_timestamps + n_extra_fields + 2 + k;
            uint32_t _distance_idx = min(
                uint32_t(current_seq - meta->_past_timestamp) / edc_windows[k],
                max_hash_edc_idx
            );
            if (meta->_extra)
                data[idx_feature++] =
                    meta->_extra->_edc[k] * hash_edc[_distance_idx];
            else
                data[idx_feature++] = hash_edc[_distance_idx];
        }

        // 如果启用新特征，添加成本/大小比率和频次特征
        if (add_new_features) {
            indices[idx_feature] =
                max_n_past_distances + n_extra_fields + n_edc_feature + 2;
            data[idx_feature++] =
                static_cast<double>(meta->_cost) /
                static_cast<double>(meta->_size);
        ...
        // 更新indptr数组，记录当前行的特征数量
        indptr[idx_row + 1] = idx_feature;
        idx_row++;
    }

    system_clock::time_point admit_timeBegin2 = chrono::system_clock::now();

    // 进行批量预测
    double scores[sample_size];
    int64_t len;
    LGBM_BoosterPredictForCSR(
        booster,
        static_cast<void *>(indptr),
    ...
    admit_inference+=sample_metas.size();

    double tmp_admit_inference_time2 = chrono::duration_cast<chrono::microseconds>(chrono::system_clock::now() - admit_timeBegin2).count();
    admit_inference_time += tmp_admit_inference_time2;
    total_inference_time += tmp_admit_inference_time2;

    for (uint32_t i = 0; i < sample_metas.size(); ++i) {
        scores[i] += log1p(sizes[i]) - log1p(costs[i]);
    }
    ...
    // 如果新对象的分数小于等于采样对象的最大分数，则接纳
    return score_new < max_sample_score;
}
