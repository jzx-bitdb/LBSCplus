...
FrameWork::FrameWork(const string &trace_file, const string &cache_type, const uint64_t &cache_size,
    ...
    if (is_offline) {
        cerr<<"offline_num:"<<_offline_num<<endl;
        annotate(_trace_file, n_extra_fields, _offline_num, _offline_threshold, n_early_stop);
    } 
    ...
    if(is_cost){
        ...
        annotate_cost(_trace_file, n_extra_fields, is_offline, _offline_num,_delta_ratio, _fixed_byte, _min_ratio, n_early_stop);
    }
    ...
    webcache = move(Cache::create_unique(_cache_type));
    if (webcache == nullptr) throw runtime_error("Error: cache type " + _cache_type + " not implemented");
...
void FrameWork::update_stats() {
    ...
    cerr << "segment total_get_cost: " << (interval_get_time / (1000.0 * 1000.0 * 60.0)) << endl;
...
void FrameWork::simulate() {
    ...
    while (true) {
        ...
        if (is_admitting) {
            // cout<<"seq:"<<seq<<endl;
            // cout<<"t:"<<t<<" id:"<<id<<"  size:"<<size<<endl;
            // cout<<"req->size:"<<req->size<<endl;
            bool is_hit = webcache->lookup(*req);

            // cerr << "[DGB]before hit cost: " << req->cost << " interval_get_time: " << interval_get_time <<endl;
            ...
            if (!is_hit) {
                ...
                // cerr << "[DGB] cost: " << req->cost << " interval_get_time: " << interval_get_time <<endl;
...
void FrameWork::simulation_results() {
    ...
    cout<<"total_get_cost/min:"<<total_time_min<<endl;
    ...
    cout<<"total_get_cost_no_threshold/min:"<<total_time_min_no_threshold<<endl;
    ...
    //                                                                              seg_byte_req.end(), 0)
    // ));
    cout<<"no_warmup_byte_miss_ratio:"<<
        accumulate<vector<int64_t>::const_iterator, double>(seg_byte_miss.begin(), seg_byte_miss.end(), 0) /
        accumulate<vector<int64_t>::const_iterator, double>(seg_byte_req.begin(), seg_byte_req.end(), 0);
    ...
    //     ));
    cout<<"no_warmup_object_miss_ratio:"<<
        accumulate<vector<int64_t>::const_iterator, double>(seg_object_miss.begin(), seg_object_miss.end(), 0) /
        accumulate<vector<int64_t>::const_iterator, double>(seg_object_req.begin(), seg_object_req.end(), 0);

    //     value_builder.append(kvp("segment_byte_miss", [this](sub_array child) {
    //         for (const auto &element : seg_byte_miss)
    //             child.append(element);
    //     }));
    cout<<"segment_byte_miss:"<<endl;
    for (const auto &element : seg_byte_miss){
        cout<<element<<" ";
    }
    cout<<endl;
    ...
    //             child.append(element);
    //     }));
    cout<<"segment_byte_in_cache:"<<endl;
    for (const auto &element : seg_byte_in_cache){
        cout<<element<<" ";
    }
    cout<<endl;
    webcache->update_stat();
...
void _simulation(string trace_file, string cache_type, uint64_t cache_size,
...
void _simulation_hro(string trace_file, string cache_type, uint64_t cache_size,
                                              map<string, string> params) {
...
void simulation(string trace_file, string cache_type,
    ...
    // int n_extra_fields = 0;
    // params["n_extra_fileds"] = "0";
    ...
    if (cache_type == "Adaptive-TinyLFU")
        _simulation_tinylfu(trace_file, cache_type, cache_size, params);
    else if(cache_type == "HRO" || cache_type == "HROCost")
        _simulation_hro(trace_file, cache_type, cache_size, params);
    else
        _simulation(trace_file, cache_type, cache_size, params, req_num, max_data_size);
}
