...
// #include "mongocxx/client.hpp"  //去掉mongo db相关的内容  所以就不用额外编译mongodb了
...
int main(int argc, char *argv[]) {
    ...
    // ./webcachesim_cli traceFile cacheType cacheSize [--param=value] 最少4个参数
    if (argc < 4) {
        cerr
    ...
    for(int i=4; i<argc; i++) {
        ...
        params[opmatch[1]] = opmatch[2];// 一些optional parameters
    }


    auto webcachesim_trace_dir = getenv("WEBCACHESIM_TRACE_DIR"); //必须有这个环境变量
    ...
    simulation(string(webcachesim_trace_dir) + '/' + argv[1], argv[2], std::stoull(argv[3]),
                          params);
    ...
    //输出 cache算法所占用时间 以及当前时间戳
    cout<< "simulation_time: " << simulation_time << endl;
    cout<< "simulation_timestamp: " << simulation_timestamp << endl;
