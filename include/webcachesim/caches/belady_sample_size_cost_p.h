#ifndef F4019924_AD09_4ED4_886A_03F5C31C0DEC
#define F4019924_AD09_4ED4_886A_03F5C31C0DEC
#ifndef WEBCACHESIM_BELADY_SAMPLESIZECOSTP_H
#define WEBCACHESIM_BELADY_SAMPLESIZECOSTP_H
...
class BeladySampleSizeCostCacheP : public Cache
{
public:
    ...
    uint64_t cost_threshold = 0.01;
    int64_t exceed_threshold = 0;
    int64_t total_exceed_threshold = 0.5;
    ...
    double eviction_num = 0.0;
    double admit_bypass_num = 0.0;
    ...
    void update_stat() override {
        ...
        cout<<"total_exceed_threshold:"<<total_exceed_threshold<<endl;
        cout<<"total_max_rank:"<<(int64_t)total_max_rank<<endl;
        cout<<"total_min_rank:"<<(int64_t)total_min_rank<<endl;
        cout<<"total_evict_max_size_cost_rate:"<<to_string(total_evict_max_size_cost_rate)<<endl;
        cout<<"total_evict_min_size_cost_rate:"<<to_string(total_evict_min_size_cost_rate)<<endl;


        cerr << "admit_bin: ";
        for (size_t i = 0; i < g_admit_bins.size(); i++) {
            cerr << g_admit_bins[i] << " ";
        }
        cerr << endl;

        cerr << "pass_bin: ";
        for (size_t i = 0; i < g_pass_bins.size(); i++) {
            cerr << g_pass_bins[i] << " ";
        }
        cerr << endl;
    }
    ...
    void update_stat_periodic() override {
        ...
        cerr<<"eviction_num:"<<eviction_num<<endl;
        cerr<<"admit_bypass_num:"<<admit_bypass_num<<endl;
        

        eviction_num=0.0;
        admit_bypass_num=0.0;
    ...
    bool rank_admit(const SimpleRequest &req);
};

static Factory<BeladySampleSizeCostCacheP> factoryBeladySampleSizeCostP("BeladySampleSizeCostP");



#endif //WEBCACHESIM_BELADY_SAMPLESIZECOSTP_H


#endif /* F4019924_AD09_4ED4_886A_03F5C31C0DEC */
