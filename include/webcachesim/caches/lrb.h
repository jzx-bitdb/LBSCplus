...
struct MetaExtra {
    ...
    MetaExtra(const uint32_t &distance) {
        ...
        for (uint8_t i = 0; i < n_edc_feature; ++i) {  
            ...
            _edc[i] = hash_edc[_distance_idx] + 1; //past request distance??
        }
    }

    void update(const uint32_t &distance) {//更新 past request distance
        ...
        if (_past_distances.size() < max_n_past_distances)
            ...
            _past_distances[distance_idx] = distance; //只存储过去 固定次数的访问情况，再往前的访问就不考虑了
...
class Meta { //存储每个object 的类
public:
    //25 byte
    uint64_t _key; //8
    uint32_t _size; // 4
    uint32_t _past_timestamp; // 4
    uint16_t _extra_features[max_n_extra_feature];  //存储extra features
    ...
    void update(const uint32_t &past_timestamp) {
        ...
        if (!_extra) {
            ...
            _extra->update(_distance); //更新每个object的 请求访问序列
        //timestamp
        _past_timestamp = past_timestamp;
    }
#endif
    int feature_overhead() { //记录feature 所占的空间
        int ret = sizeof(Meta);
        if (_extra)
...
public:
    ...
    list<int64_t>::const_iterator request(int64_t key) {
        dq.emplace_front(key);//每次请求命中cache  对应的request 更新到队列头部
        return dq.cbegin();
    }
...
public:
    ...
    void emplace_back(Meta &meta, uint32_t &sample_timestamp, uint32_t &future_interval, const uint64_t &key) {
        ...
        data.emplace_back(sample_timestamp - meta._past_timestamp); //data 用来存储feature
        ++counter;
        ...
        indices.push_back(max_n_past_timestamps + n_extra_fields + 1); //indices存的是feature 对应的索引位置
        ...
        labels.push_back(log1p(future_interval));  //label 用 log的形式
...
//    vector<uint16_t> training_and_prediction_logic_timestamps; 
    ...
    void init_with_params(const map<string, string> &params) override { //初始化模型的一些参数  这个可以不变化
        for (auto &it: params) {
...
//     }
    ...
    void update_stat() override {
        ...
        cout<<"n_metadata:"<<static_cast<int32_t>(key_map.size())<<endl;
        cout<<"feature_overhead:"<<feature_overhead<<endl;
        cout<<"sample_overhead:"<<sample_overhead<<endl;
        ...
        auto v = get_object_distribution_n_past_timestamps();
        cout<<"object_distribution_n_past_timestamps:"<<endl;
        for (const auto &element : v){
            cout<<element<<", ";
        }
        cout<<endl;

        cout<<"segment_percent_beyond:"<<endl;
        for (const auto &element : segment_percent_beyond){
            cout<<element<<", ";
        }
        cout<<endl;

        cout<<"segment_positive_example_ratio:"<<endl;
        for (const auto &element : segment_positive_example_ratio){
            cout<<element<<", ";
        }
        cout<<endl;
    ...
    vector<int> get_object_distribution_n_past_timestamps() { //过去访问频率的分布情况
