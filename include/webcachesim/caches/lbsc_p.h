#ifndef D3DB8151_CA82_47DF_B60C_D6943C33CB75
#define D3DB8151_CA82_47DF_B60C_D6943C33CB75
#ifndef WEBCACHESIM_LBSCP_H
#define WEBCACHESIM_LBSCP_H
...
struct RankCandidate {
    ...
    KeyMapEntryT pos;
    double score;
    double predicted_interval;
    uint32_t infer_seq;
};

struct JointRankResult {
    bool admit_new;                             // meta_new != nullptr 时：是否接纳新对象
    pair<uint64_t, KeyMapEntryT> victim;   // 纯 eviction 场景用：要驱逐的 (key, pos)
};

class LBSCCacheP : public Cache{
public:
    ...
    int bypass_num = 0;
    int admit_bypass_num = 0;
    
    int admit_inference = 0;
    int eviction_num = 0;

    vector<uint64_t> g_admit_bins;
    vector<uint64_t> g_pass_bins;
    ...
    vector<RankCandidate> cached_candidates;
    bool cached_first_reuse_done = false;  // 是否完成第一次复用
    bool model_update = false; //是否更新了模型
    uint32_t tmp_rank_num = 0;
    ...
    void evict_single(uint64_t key, const KeyMapEntryT &old_pos);
    ...
    bool rank_admit(const shared_ptr<Meta>& meta_new, const SimpleRequest &req);

    JointRankResult joint_rank(const shared_ptr<Meta>& meta_new, const SimpleRequest &req);
    ...
    void init_with_params(const map<string, string> &params) override {
        for (auto &it: params) {
            ...
            else if (it.first == "reuse_rate") {
                reuse_rate = stoul(it.second);
                cerr << "reuse_rate: " << reuse_rate <<endl;
            }
    ...
    void update_stat() override {
        total_training_time /= 1000;
        total_inference_add_feature_time /= 1000000;
        total_inference_time /= 1000000;
        total_inference_sort_time /= 1000000;
        cout<<"total_training_time:"<<to_string(total_training_time)<<endl;
        cout<<"total_inference_time:"<<to_string(total_inference_time)<<endl;
        cout<<"total_add_feature_inference_time:"<<to_string(total_inference_add_feature_time)<<endl;
        cout<<"total_inference_sort_time:"<<to_string(total_inference_sort_time)<<endl;
        cout<<"n_force_eviction:"<<n_force_eviction<<endl;
        cout<<"min_js:"<<to_string(min_js)<<endl;
        cout<<"max_js:"<<to_string(max_js)<<endl;
        ...
        cout<<"segment_n_in:"<<endl;
        for (const auto &element:segment_n_in){
            cout<<element<<",";
        }
        cout<<endl;

        cout<<"segment_n_out:"<<endl;
        for (const auto &element:segment_n_out){
            cout<<element<<",";
        }
        cout<<endl;

        cout<<"segment_n_retrain:"<<endl;
        for (const auto &element:segment_n_retrain){
            cout<<element<<",";
        }
        cout<<endl;

        cout<<"model_importance:"<<endl;
        for (const auto &element:importances){
            cout<<element<<",";
        }
        cout<<endl;
...
static Factory<LBSCCacheP> factoryLBSC("LBSCP");
}
#endif //WEBCACHESIM_LBSC_H


#endif /* D3DB8151_CA82_47DF_B60C_D6943C33CB75 */
